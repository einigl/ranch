...
def kde(obj : 'struct.Struct', axis : Optional[Literal['spatial', 'spectral']] = None,
    h : Optional[Union[float, np.ndarray]] = None, t : Optional[np.ndarray] = None,
    t_bounds : Optional[Tuple[float]] = None, t_step : float = 0.05) -> Tuple[np.ndarray] :
    ...
    obj : Cube | Map | Profile
        Input structure.
    axis : str, optional
        Axis over which to compute the PDF. Must be set only if obj is an instance of Cube.
        If axis is None, the PDF is computed over all the flattened structure so the output
        `pdf` is a 1D-array
    h : float or ndarray, optional
        Kernel parameter. Must be a scalar if obj is an instance of Map or Profile.
        If obj is an instance of Cube, `h` must be an array of shape (nz,) if axis == 'spectral'
    ...
    pdf : ndarray
        Estimated PDF.
    """
    if axis is not None and not isinstance(obj, struct.Cube) :
        raise ValueError('axis must only be set if obj is an instance of Cube')
    elif axis is not None and isinstance(obj, struct.Cube) :
        axis = axis.lower()
        if axis == 'spectral' :
            data = np.moveaxis(obj.data, 0, -1)
        elif axis == 'spatial' :
            data = obj.data.reshape(obj.nz, -1)
        else :
