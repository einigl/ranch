...
__all__ = [
    'default_cmap', 'plot_channel', 'plot_hist', 'plot_hist2d', 'plot_map', 'plot_pixel', 'plot_profile', 'save_channel_plot', 'save_hist', 'save_hist2d', 'save_map_plot', 'save_pixel_plot', 'save_profile_plot', 'show_channel', 'show_hist', 'show_hist2d', 'show_map', 'show_pixel', 'show_profile'
]
...
def _load_fits(filename : str, path : Optional[str] = None) -> Tuple[np.ndarray, fits.Header] :
    ...
    if os.path.exists(file + '.fits') :
        ...
        raise FileNotFoundError('No files with a .fits or .fits.gz extension were found for the path {}'\
            .format(file))
    hdu = fits.open(file + ext)[0]
    return hdu.data, hdu.header

def _save_fits(obj : 'struct.Struct', filename : str, path : Optional[str] = None, overwrite = False) -> None :
    """ Save `obj` structure in a FITS file. The extension of the file can be ommited. """
    if path is not None and not os.path.exists(path) :
        os.makedirs(path)
    file = filename if path is None else os.path.join(path, filename)
    file = file.replace('.fits', '').replace('.gz', '') + '.fits'
    hdu = fits.PrimaryHDU(obj.data, obj.header)
...
def plot_map(map : 'struct.Map', ax : Optional[plt.Axes] = None, label_unit : str = 'angle',
    ...
    if map.header['CTYPE1'].upper() == 'INDICES' or map.header['CTYPE2'].upper() == 'INDICES' :
        label_unit = 'index'
    ...
    if label_unit == 'angle' :
        ...
        imshow_kwargs.update({
            ...
            'aspect' : map.shape[0] / map.shape[1]\
...
def plot_profile(profile : 'struct.Profile', ax : plt.Axes = None,
    label_unit : str = 'velocity', logy : bool = False,
    linestyle : Optional[str] = 'solid', color : Optional[str] = 'k',
    linewidth : Optional[float] = 1.5, label : Optional[str] = None) :
    ...
    if label_unit == 'velocity' :
        line = plot(hdr.index_to_velocity(profile.header, idx), profile.data, **plot_kwargs)
        plot_label("Velocity (km/s)")
    elif label_unit == 'frequency' :
        line = plot(hdr.index_to_frequency(profile.header, idx), profile.data, **plot_kwargs)
...
def plot_channel(cube : 'struct.Cube', z : Union[int, float], unit : str = 'index', **kwargs) :
    """ TODO """
    return cube.get_channel(z, unit = unit).plot(**kwargs)

def plot_pixel(cube : 'struct.Cube', xy : tuple[int, float], unit : str = 'index', **kwargs) :
    """ TODO """
    return cube.get_pixel(xy, unit = unit).plot(**kwargs)

def save_map_plot(map : 'struct.Map', filename : str, path : Optional[str] = None, **kwargs) -> None :
    """ TODO """
    plot_map(map, **kwargs)
    if path is None:
...
def save_profile_plot(profile : 'struct.Profile', filename : str, path : Optional[str] = None, **kwargs) -> None :
    """ TODO """
...
def save_channel_plot(cube : 'struct.Cube', z : Union[int, float], filename : str,
    unit : str = 'index', path : Optional[str] = None, **kwargs) -> None :
    """ TODO """
    map = cube.get_channel(z, unit = unit)
    save_map_plot(map, filename, path, **kwargs)

def save_pixel_plot(cube : 'struct.Cube', xy : Union[tuple[int], tuple[float]], filename : str,
    unit : str = 'index', path : Optional[str] = None, **kwargs) -> None :
    """ TODO """
...
def show_channel(cube : 'struct.Cube', z : Union[int, float],
    unit : str = 'index', **kwargs) -> None :
    """ TODO """
    map = cube.get_channel(z, unit = unit)
    show_map(map, **kwargs)

def show_pixel(cube : 'struct.Cube', xy : Union[tuple[int], tuple[float]],
    unit : str = 'index', **kwargs) -> None :
    """ TODO """
    profile = cube.get_pixel(xy, unit = unit)
...
def plot_hist(obj : 'struct.Struct', bins : Union[int, str] = 'auto', ax : Optional[plt.Axes] = None,
    logx : bool = False, logy : bool = False, density : bool = False, color : Optional[str] = None,
    xlim : Optional[tuple] = None, ylim : Optional[tuple] = None) -> Tuple[np.ndarray, np.ndarray, plt.Axes] :
    """ TODO """
    a = obj.data.flatten()
    a = a[~np.isnan(a)]
...
def plot_hist2d(obj1 : 'struct.Struct', obj2 : 'struct.Struct', bins : Union[int, tuple, str] = 'auto',
    ax : Optional[plt.Axes] = None, logx : bool = False, logy : bool = False, logz : bool = False,
    density : bool = False, plot_identity : bool = False, cmap : Union[str, colors.Colormap] = default_cmap,
    scatter : bool = False, scatter_threshold : int = 1, zeros_to_nans : bool = True,
    xlim : Optional[tuple] = None, ylim : Optional[tuple] = None,
    vmin : float = None, vmax : float = None) -> Tuple[np.ndarray, Tuple[np.ndarray, np.ndarray], plt.Axes]:
    """
    Plot a 2-dimensional histogram of two structures of same shape `obj1` and `obj2`.

    Parameters
    ----------
    obj1 : `Cube | Map | Profile`.
        Structure whose data will be plotted on the x axis.
    obj2 : `Cube | Map | Profile`.
    ...
    if obj1.shape != obj2.shape :
        raise ValueError(f'obj1 and obj2 must have the same shape (here {obj1.shape} and {obj2.shape})')
    ...
    if xlim is None :
        ...
        _xlim = (a.min() if xlim[0] is None else xlim[0],
                a.max() if xlim[1] is None else xlim[1])
    if ylim is None :
        _ylim = (b.min(), b.max())
    else :
        _ylim = (b.min() if ylim[0] is None else ylim[0],
    ...
    if isinstance(bins, str) :
        ...
        if bins <= 0 :
            raise ValueError('bins must be a positive integer, a tuple of two positive integers or a string')
        bins = (bins, bins)
    elif isinstance(bins, tuple) :
        if len(bins) != 2 or bins[0] <= 0 or bins[1] <= 0 :
            raise ValueError('bins must be a positive integer, a tuple of two positive integers or a string')
    else :
        raise TypeError(f'bins must be an integer, a tuple of integer or a string, not {type(bins)}')
    ...
    if scatter :
        ...
        for i, j in product(range(xedges.size-1), range(yedges.size-1)) :
            if (0 < H_count[i, j]) and (H_count[i, j] <= scatter_threshold) :
                _cond = (xedges[i] <= a) & (a <= xedges[i+1]) & (yedges[j] <= b) & (b <= yedges[j+1])
    ...
    im = ax.pcolormesh(X, Y, H.T, norm = colors.LogNorm(vmin, vmax) if logz else None,
                       cmap = cmap)
...
def save_hist(obj : 'struct.Struct', filename : str, path : Optional[str] = None, **kwargs) :
    """ TODO """
    plot_hist(obj, **kwargs)
    plt.savefig(os.path.join(path, filename))

def save_hist2d(obj1 : 'struct.Struct', obj2 : 'struct.Struct', filename : str, path : Optional[str] = None, **kwargs) :
    """ TODO """
    plot_hist2d(obj1, obj2, **kwargs)
    plt.savefig(os.path.join(path, filename))

def show_hist(obj : 'struct.Struct', **kwargs) :
    """ TODO """
    plot_hist(obj, **kwargs)
    plt.show()

def show_hist2d(obj1 : 'struct.Struct', obj2 : 'struct.Struct', **kwargs) :
    """ TODO """
    plot_hist2d(obj1, obj2, **kwargs)
    plt.show()
