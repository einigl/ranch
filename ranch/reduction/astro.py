...
def integral(cube : 'struct.Cube', ppv_mask: Optional['struct.Cube']=None, ignore_nans: bool=True) -> 'struct.Map' :
    """
    ...
    if ppv_mask is not None:
        cube = ppv_mask * cube
    return cube.sum(struct.Map, ignore_nans=ignore_nans) * cube.header['CDELT3'] / 1e3 # Convert to km/s
...
def noise_map(cube : 'struct.Cube', signal_mask : Union[slice, List[slice]], unit = 'index') -> 'struct.Map' :
    """
    ...
    for mask in signal_mask :
        ...
        if unit == 'index' :
            ...
            index_mask = slice(hdr.velocity_to_index(cube.header, mask.start),\
                hdr.velocity_to_index(cube.header, mask.stop))
        elif unit == 'frequency' : # Order is reversed for frequencies
            index_mask = slice(hdr.frequency_to_index(cube.header, mask.stop)-1,\
...
@overload
def reduce_spectral(obj : 'struct.Cube', z_interv : Optional[slice] = None) -> 'struct.Cube' : ...
@overload
def reduce_spectral(obj : 'struct.Profile', z_interv : Optional[slice] = None) -> 'struct.Profile' : ...

def reduce_spectral(obj : Union['struct.Cube', 'struct.Profile'], z_interv : Optional[slice] = None,
    unit : str = 'index') -> Union['struct.Cube', 'struct.Profile'] :
    """
    ...
    if unit == 'index' :
        z = [
            z_interv.start or 0,
            (z_interv.stop or obj.nz)-1
        ]
    elif unit == 'velocity' :
        z = [
            hdr.velocity_to_index(z_interv.start) if z_interv.start is not None else 0,
            (hdr.velocity_to_index(z_interv.stop) if z_interv.stop is not None else obj.nz)-1
        ]
    elif unit == 'frequency' : # Order is reversed for 'frequency' unit
        z = [
            (hdr.frequency_to_index(z_interv.stop)  if z_interv.stop is not None else obj.nz)-1,
            hdr.frequency_to_index(z_interv.start) if z_interv.start is not None else 0
        ]
    
    new_data = obj.data[z_interv]
    new_header = hdr.change_coordinates(obj.header, z=z)
    return type(obj)(new_data, new_header)

@overload
def reduce_spatial(obj : 'struct.Cube', x_interv : Optional[slice] = None,
    y_interv : Optional[slice] = None) -> 'struct.Cube' : ...
@overload
def reduce_spatial(obj : 'struct.Map', x_interv : Optional[slice] = None,
    y_interv : Optional[slice] = None) -> 'struct.Map' : ...

def reduce_spatial(obj : Union['struct.Cube', 'struct.Map'], x_interv : Optional[slice] = None,
    y_interv : Optional[slice] = None, unit : str = 'index') -> Union['struct.Cube', 'struct.Map'] :
    """
    ...
    if unit == 'index' :
        ...
        x_start, y_start = hdr.coordinates_to_indices(obj.header, x_interv.start, y_interv.start)
        x_stop, y_stop = hdr.coordinates_to_indices(obj.header, x_interv.stop, y_interv.stop)
        x = [x_start, x_stop-1]
        y = [y_start, y_stop-1]

    new_data = obj.data[..., y_interv, x_interv]
    new_header = hdr.change_coordinates(obj.header, x = x, y = y)
    return type(obj)(new_data, new_header)
